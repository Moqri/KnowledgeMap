...
function setupGUI(config) {
   ...
   // $GP.info_close.click(nodeNormal);//->juni [req REQ_004] - When details for a node are closed (so when we have filters applied), all nodes appear which is not correct.
    $GP.info_close2.click(nodeNormal);
    $GP.form = $("#mainpanel").find("form");
    $GP.search = new Search($GP.form.find("#search"));
	//->juni [REQ_004]
	$GP.year = new YearFilter($GP.form.find("#year_filter_div"));//initiate our new filter
    $GP.info_close.click( function(){nodeWithFilter()});// on close of details (right side)
	//<-juni [REQ_004]
...
function configSigmaElements(config) {
	...
	//-> juni [req REQ_004] - 2014-10-08 - remove loading indicator after 3 seconds
	setTimeout(function () {
		 jQuery('#loading-trans').remove();
	}, 3000);
	//<-juni [req REQ_004]
}
...
function Search(a) {
    ...
    this.search = function (a) {
        ...
        if (2 >= a.length) this.results.html("<i>You must search for a name with a minimum of 3 letters.</i>");
			...
			//juni -> this is the original code
			/*
            ...
            if (1 < c.length) for (var d = 0, h = c.length; d < h; d++) {//moqri/ add odd class to style the odd results
				if (d%2) a.push('<a class="odd" href="#' + c[d].name + '" onclick="nodeActive(\'' + c[d].id + "')\">" + c[d].name + "</a>");
				else a.push('<a href="#' + c[d].name + '" onclick="nodeActive(\'' + c[d].id + "')\">" + c[d].name + "</a>");
			}
            0 == c.length && !b && a.push("<i>No results found.</i>");
            1 < a.length && this.results.html(a.join(""));
			*/
			//juni -> 2014-09-08 -> add year and source filter
			//clear all results before searching
			$GP.search.results.hide();   
			$GP.info.delay(400).animate({width:'hide'},350);
			$GP.cluster.hide();
			//;
			var yMin =  jQuery("#year_min").val();//selected year min/max
			var yMax =  jQuery("#year_max").val();	
			var source_filter = "";
			var srcReg = "";		
			//iterate over check source types 
			$('#source_buttons label').each(function(){
				var elFor = $(this)[0].htmlFor;
				if (elFor!='') {
					if ($(this).hasClass('selected'))  {
						var filter = $(this).find('img').prop('alt');
						if (filter!='')
							source_filter += filter+"|"
					}
				}
			});
			if (source_filter)//remove last |
					source_filter = source_filter.substr(0,source_filter.length-1)
			if (source_filter != '')
					srcReg = RegExp(this.exactMatch ? ("^" + source_filter + "$").toLowerCase() : source_filter.toLowerCase());
			else 
					srcReg = RegExp("$7$");//if nothing is select, enter dummy regex that can't matching anything
			console.log("Search using YearMin:"+yMin + ", YearMax:"+yMax + ", Sources:"+srcReg + ", SearchedTerm:"+g);					
            sigInst.iterNodes(function (a) {//iterate over all nodes to find matches
				//hide others 
				a.hidden = !0;
				a.attr.lineWidth = !1;
				a.attr.color = a.color
				if (a.attr.attributes.Year >= yMin && a.attr.attributes.Year <= yMax &&srcReg.test(a.attr.attributes.Source.toLowerCase()) ) {
					if (g.test(a.label.toLowerCase())) {
						a.hidden = !1;
						a.attr.color = a.color;
						a.attr.lineWidth = 6;
						a.attr.strokeStyle = "#000000";
						//add to results
						c.push({ 
							id: a.id,
							name: a.label
			...
			sigInst.draw();
			//results
            a = ["<b style='padding:2px;margin-left:30px;'>Search Results: ("+c.length+")</b><br/>"];
			//2014-10-10 -> display a single record if that is the result
            //if (1 < c.length) for (var d = 0, h = c.length; d < h; d++) {//odd class add to look nicer
            if (1 <= c.length) for (var d = 0, h = c.length; d < h; d++) {
				if (d%2)
					a.push('<a class="odd" href="#' + c[d].name + '" onclick="nodeActive(\'' + c[d].id + "')\">" + c[d].name + "</a>");
				else
					a.push('<a href="#' + c[d].name + '" onclick="nodeActive(\'' + c[d].id + "')\">" + c[d].name + "</a>");
			} else if (1==c.length) { //15.09.2014 - When a single record is filtered, display the right information panel
				nodeActive(c[0].id);
			} else
				; //do nothing ;)
			// 0 == c.length  && a.push("<i>No results found.</i>");//juni - 2014-10-11 -> if there is no result, display this
            0 == c.length  && a.push("<i>No results found.</i>") && c.push({id:0,name:'No results'});
            1 < a.length && this.results.html(a.join(""));;			
           }
		   //<-juni
     /*    if(c.length!=1) this.results.show();
		...
		//2014-10-10 -> display a single record if that is the result
		if(c.length!=0) this.results.show();
        if(c.length==0) this.results.hide();   
    }
}

//->juni [req REQ_001-REQ_002]
/**
Filter by year
	@param container
*/
function YearFilter(a) {
    this.input = a.find("input[name=year_reg]");
    this.state = $(document).find("#search_filter_state");//use the search filter to display results
    this.results = $(document).find("#search .results");//use the search filter to display results
    this.exactMatch = !1;
    this.lastSearch = "";
    this.searching = !1;
    var b = this;
    ...
    this.search = function (yMin,yMax) {
        var  c = [];
        this.lastSearch = yMin;
        this.results.empty();
        if (jQuery.isEmptyObject(yMin) || jQuery.isEmptyObject(yMax)) { //take last values
			var yMin =  jQuery("#year_min").val();
			var yMax =  jQuery("#year_max").val();		
		}
        if (yMin== ''&& yMax =='') this.results.html("<i>You must select a year to filter!</i>");
        else {
			//clear all results before searching
			$GP.search.results.hide();   
			$GP.info.delay(400).animate({width:'hide'},350);
			...
			var source_filter = "";
			var srcReg = "";		
			//2014-10-10 -> when filtering look at searched text also
			var searchFilter =  jQuery("#search input[name=search]").val();
			var searchReg = "";			
			//iterate over check other_filter types 
			$('#source_buttons label').each(function(){
				var elFor = $(this)[0].htmlFor;
				if (elFor!='') {
					if ($(this).hasClass('selected'))  {
			...
			//2014-10-10 -> when filtering look at searched text also
			if (searchFilter != "")
				searchReg = RegExp(this.exactMatch ? ("^" + searchFilter + "$").toLowerCase() : searchFilter.toLowerCase());
			else 
				searchReg = RegExp("");								
			console.log("Year filtering using YearMin:"+yMin + ", YearMax:"+yMax + ", Sources:"+srcReg + ", SearchedTerm:"+searchFilter);							
			sigInst.iterNodes(function (a) {
				a.hidden = !0;
				a.attr.lineWidth = !1;
				a.attr.color = a.color;
				//if (a.attr.attributes.Year >= yMin && a.attr.attributes.Year <= yMax &&srcReg.test(a.attr.attributes.Source.toLowerCase())) {
				if (a.attr.attributes.Year >= yMin && a.attr.attributes.Year <= yMax &&srcReg.test(a.attr.attributes.Source.toLowerCase()) && searchReg.test(a.label.toLowerCase()) ) { 	//2014-10-10 -> when filtering look at searched text also
					a.hidden = !1;
					a.attr.color = a.color;
					a.attr.lineWidth = 6;
					a.attr.strokeStyle = "#000000";
...
/**
	[req REQ_002] - When details for a node are closed (so when we have filters applied), all nodes appear which is not correct.
**/
function nodeWithFilter() {
	!0 != $GP.calculating && !1 != sigInst.detail && (showGroups(!1), $GP.calculating = !0, sigInst.detail = !0, $GP.info.delay(400).animate({width:'hide'},350),$GP.cluster.hide(),
	sigInst.neighbors = {}, sigInst.active = !1, $GP.calculating = !1, window.location.hash = "");
	$GP.year.search($("#year_min").val(),$("#year_max").val());//filter by year (which in turn will filter data)
}
//<-juni [req REQ_002]
...
function nodeActive(a) {
    ...
    d = "";
		for (g in e) {
			...
			if (g%2) f.push('<tr><td class="trodd"><a class="tdstyle" href="#' + c.name + '" onmouseover="sigInst._core.plotter.drawHoverNode(sigInst._core.graph.nodesIndex[\'' + c.id + '\'])\" onclick=\"nodeActive(\'' + c.id + '\')" onmouseout="sigInst.refresh()">' + c.name + "</a></td></tr>");
		            else f.push('<tr><td class="membership"><a class="tdstyle" href="#' + c.name + '" onmouseover="sigInst._core.plotter.drawHoverNode(sigInst._core.graph.nodesIndex[\'' + c.id + '\'])\" onclick=\"nodeActive(\'' + c.id + '\')" onmouseout="sigInst.refresh()">' + c.name + "</a></td></tr>");
    ...
    $GP.info_link.find("table").html(f.join(""));
    $GP.info_link.find("tr").each(function () {
        var a = $(this),
            b = a.attr("rel");
    });
    f = b.attr;
...
function showCluster(a) {
    ...
    if (b && 0 < b.length) {
        ...
        for (var f = [], e = [], c = 0, g = b.length; c < g; c++) {
           ...
           if(c%2) !0 == d.hidden && (e.push(b[c]), d.hidden = !1, d.attr.lineWidth = !1, d.attr.color = d.color, f.push('<tr><td class="trodd" ><a class="tdstyle" href="#'+d.label+'" onmouseover="sigInst._core.plotter.drawHoverNode(sigInst._core.graph.nodesIndex[\'' + d.id + "'])\" onclick=\"nodeActive('" + d.id + '\')" onmouseout="sigInst.refresh()">' + d.label + "</a></td></tr>"))
            else !0 == d.hidden && (e.push(b[c]), d.hidden = !1, d.attr.lineWidth = !1, d.attr.color = d.color, f.push('<tr><td class="membership" ><a class="tdstyle" href="#'+d.label+'" onmouseover="sigInst._core.plotter.drawHoverNode(sigInst._core.graph.nodesIndex[\'' + d.id + "'])\" onclick=\"nodeActive('" + d.id + '\')" onmouseout="sigInst.refresh()">' + d.label + "</a></td></tr>"))
